...
class Routes {
  Router get router {
    ...
    router.get('/<domain>/', domainController.index);
    router.get('/<domain>/issue/<issueId>/articles/', issueController.getArticles);
    router.get('/<domain>/article/<articleId>/', articleController.show);
    router.get('/<domain>/archive/', archiveController.index);
    router.get('/<domain>/archive/volume/<volumeId>/issues/', archiveController.showVolumeIssues);
    ...
    router.get('/<domain>/editorial-board/', journalInfoController.editorialBoard);
    router.get('/<domain>/publication-ethics/', journalInfoController.publicationEthics);
    router.get('/<domain>/indexing-and-abstracting/', journalInfoController.indexingAndAbstracting);
    router.get('/<domain>/peer-review-process/', journalInfoController.peerReviewProcess);

    // Author Resources Routes
    router.get('/<domain>/submit-online-paper/', authorResourcesController.submitOnlinePaper);
    router.get('/<domain>/topics/', authorResourcesController.topics);
    router.get('/<domain>/author-guidelines/', authorResourcesController.authorGuidelines);
    router.get('/<domain>/copyright-form/', authorResourcesController.copyrightForm);
    router.get('/<domain>/check-paper-status/', authorResourcesController.checkPaperStatus);
    router.get('/<domain>/submit-manuscript/', authorResourcesController.submitManuscript);
